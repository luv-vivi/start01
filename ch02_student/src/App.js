...
//학생 정보 시스템 메인 컴포넌트
const StudentApp=()=>{
  //현재 모든 상태관리 (현재 화면 상태) (HOME, CREATE, SELECT, UPDATE,DELDTE)
  ...
  const handleCreate = (student) => {
    let newStudents=[...students, { ...student, id: nextId }];
    setStudents(newStudents);
    setNextId(nextId + 1);

    setMode('SELECT');
    console.log(newStudents);
    // 아래와 같이 기술하면 비동기 처리 되어 딜레이가 생긴다.
...
const StudentCreate = ({ onCreate }) => {
  ...
  const handleChange = (e) => {
    ...
    //setStudent 다음에 콜백 함수(매개변수가 함수인 인자)가 오면
